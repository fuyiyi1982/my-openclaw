...
export function renderOverview(props: OverviewProps) {
  ...
  const authHint = (() => {
    ...
    if (!hasToken && !hasPassword) {
      return html`
        <div class="muted" style="margin-top: 8px">
          ${t("This gateway requires auth. Add a token or password, then click Connect.")}
          <div style="margin-top: 6px">
            <span class="mono">openclaw dashboard --no-open</span> → ${t("open the Control UI")}
            <br />
            <span class="mono">openclaw doctor --generate-gateway-token</span> → ${t("set token")}
          </div>
          <div style="margin-top: 6px">
            <a
              class="session-link"
              ...
              target="_blank"
              rel="noreferrer"
              title=${t("Control UI auth docs (opens in new tab)")}
              >${t("Docs: Control UI auth")}</a
            >
          </div>
        </div>
      `;
    ...
    return html`
      <div class="muted" style="margin-top: 8px">
        ${t("Auth failed. Update the token or password in Control UI settings, then click Connect.")}
        <div style="margin-top: 6px">
          <a
            class="session-link"
            href="https://docs.openclaw.ai/web/dashboard"
  ...
  const insecureContextHint = (() => {
    ...
    return html`
      <div class="muted" style="margin-top: 8px">
        ...
        <span class="mono">http://127.0.0.1:18789</span> ${t("on the gateway host.")}
        <div style="margin-top: 6px">
          ${t("If you must stay on HTTP, set")}
          <span class="mono">gateway.controlUi.allowInsecureAuth: true</span> (${t("token-only")}).
        </div>
        <div style="margin-top: 6px">
          <a
            class="session-link"
            ...
            target="_blank"
            rel="noreferrer"
            title=${t("Tailscale Serve docs (opens in new tab)")}
            >${t("Docs: Tailscale Serve")}</a
          >
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            title=${t("Insecure HTTP docs (opens in new tab)")}
            >${t("Docs: Insecure HTTP")}</a
          >
        </div>
      </div>
    `;
  ...
  return html`
    ...
    <section class="grid grid-cols-3" style="margin-top: 18px;">
      ...
      <div class="card stat-card">
        ...
        <div class="muted">${t("Next wake {time}", { time: formatNextRun(props.cronNext) })}</div>
      </div>
    </section>
